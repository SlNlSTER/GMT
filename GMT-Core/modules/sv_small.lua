    ...
    if user_id ~= nil then
            for k, v in pairs(smallarms.guns) do
                ...
                if vRP.tryPayment({user_id, v.price}) then
                    ...
                    local webhook = "discord webhook997200584950153307/ffq310qK-btXknlh-eYX1EeN1hvMxboGVMkybHiDqqs_0q52b5sfTimGKGDuMCOAT7jh"
             ...
             if v.gunhash == hash then
                if v.category == "smallarms" then
                    ...
                    --if vRP.tryPayment({user_id, v.price}) then
                        ...
                        local webhook = "discord webhook997200630697439343/MqXzWJktrCQs-GnuJI2jQ9MqckDGhWdxqI3BSbSL9LNggkQbh94qdfiQQSOxJm9Dmc8G"
                        PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = "GMT", embeds = command}), { ['Content-Type'] = 'application/json' }) 
                    else 
                        vRPclient.notify(source, {"~r~Insufficient funds"})
                        TriggerClientEvent("GMT:PlaySound", source, 2)
        ...
        for k, v in pairs(smallarms.guns) do
             if v.hash == hash then
                if vRP.tryPayment({user_id, v.price / 2}) then
                    ...
                    local webhook = "discord webhook997200671973589074/czKMda23MnDnYgJ7VPFI80TnDf8QCDo6d3LoqJVhcKqiZxQrgo9lX5UgzXxyi9zzZ1Tm"
                    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = "GMT", embeds = command}), { ['Content-Type'] = 'application/json' }) 
                else 
            ...
            for i,v in pairs(result) do
             if v.gunhash == hash then
                ...
                if vRP.tryPayment({user_id, tonumber(v.price / 2)}) then
                    ...
                    local webhook = "discord webhook997200745948528771/3L1VBycyJJElV6HMA-sT_t6Zi5yPUGd2Wc6FzE4nsC2g-VxgENXa24GkJvkEWWsmIAgt"
                    PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = "GMT", embeds = command}), { ['Content-Type'] = 'application/json' }) 
                else 
                    TriggerClientEvent("SmallArms:Error", source, false)
                    vRPclient.notify(source, {"~r~Insufficient funds"})
    ...
    if user_id ~= nil then
        if currentArmour < 25 then
            if vRP.tryPayment({user_id, smallarms.armourprice}) then
                ...
                local webhook = "discord webhook997200781084201002/lPxPg64UpAikQqXh79_G-e_4C4uBAMGkiFuN0a7lp0TEUC0jxiokChg7lSv16olwRjVH"
                PerformHttpRequest(webhook, function(err, text, headers) end, 'POST', json.encode({username = "GMT", embeds = command}), { ['Content-Type'] = 'application/json' }) 
            else 
                TriggerClientEvent("SmallArms:Error", source, false)
                vRPclient.notify(source, {"~r~Insufficient funds"})
