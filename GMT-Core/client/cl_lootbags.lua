...
RegisterNetEvent('GMT:LootAnim')
AddEventHandler('GMT:LootAnim', function()
    ...
    if RageUI.Visible(RMenu:Get('vRPLootBags', 'main')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true}, function()
            for i,v in pairs(ItemsInLootBag) do    
                RageUI.Button(i, "Amount: " .. v[2] .. ' KG: ' .. v[1], {}, true, function(Hovered, Active, Selected)
                    if Selected then
                        if tonumber(v[2]) > 0 then 
                            ...
                            if (GetOnscreenKeyboardResult()) then
                                ...
                                if result then 
                                    result = result
                                    GMT_server_callback('vRP:LootBagTakeItem', LootBagIDNew, result, v[3])
                                end
                            end  
                        end
                    end
